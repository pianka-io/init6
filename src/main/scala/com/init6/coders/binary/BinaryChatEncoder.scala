...
object BinaryChatEncoder extends Encoder {
  ...
  override def apply(data: Any): Option[ByteString] = {
    data match {
      case UserIn(user) =>
        SidChatEvent(0x01, user.flags, user.ping, massageName(user.name), user.client)
      case UserJoined(user) =>
        SidChatEvent(0x02, user.flags, user.ping, massageName(user.name), user.client)
      case UserLeft(user) =>
        SidChatEvent(0x03, user.flags, user.ping, massageName(user.name))
      case UserWhisperedFrom(user, message) =>
        SidChatEvent(0x04, user.flags, user.ping, massageName(user.name), message)
      case UserTalked(user, message) =>
        SidChatEvent(0x05, user.flags, user.ping, massageName(user.name), message)
      case UserBroadcast(user, message) =>
        SidChatEvent(0x06, 0, 0, INIT6, message)
      case UserChannel(user, channel, flags, _, _) =>
        SidChatEvent(0x07, flags, user.ping, massageName(user.name), channel)
      case UserFlags(user) =>
        SidChatEvent(0x09, user.flags, user.ping, massageName(user.name), user.client)
      case UserWhisperedTo(user, message) =>
        SidChatEvent(0x0A, user.flags, user.ping, massageName(user.name), message)
      ...
      case UserEmote(user, message) =>
        SidChatEvent(0x17, user.flags, user.ping, massageName(user.name), message)
      case UserFlooded =>
