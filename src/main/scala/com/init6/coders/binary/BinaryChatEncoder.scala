...
object BinaryChatEncoder extends Encoder {
  ...
  override def apply(data: Any): Option[ByteString] = {
    data match {
      case UserIn(user) =>
        SidChatEvent(0x01, user.flags, user.ping, user.name, user.statstring.map(_.toString()).getOrElse(user.client))
      case UserJoined(user) =>
        SidChatEvent(0x02, user.flags, user.ping, user.name, user.statstring.map(_.toString()).getOrElse(user.client))
      case UserLeft(user) =>
      ...
      case UserFlags(user) =>
        SidChatEvent(0x09, user.flags, user.ping, user.name, user.statstring.map(_.toString()).getOrElse(user.client))
